-- ============================================================
-- Merging duplicate member profiles
-- ============================================================
--
-- Duplicates happen when someone is added to the directory by an admin or a
-- spreadsheet import and then signs up with an invite code under an address
-- that doesn't match the one on file — the signup can't find the existing
-- profile, so it creates a second one.
--
-- Merging touches fourteen tables plus a non-FK uuid[] column, so it lives in
-- one function: a half-finished merge would leave orphaned rows with no way to
-- tell which side they belonged to, and a function body is a single
-- transaction that either lands completely or not at all.
